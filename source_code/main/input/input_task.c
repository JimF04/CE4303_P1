...
void crear_barco_por_pulsos(int pulsos)
{
    char tipo[16];
    int direccion = 0;
...
void input_task(void *arg)
{
    ...
    while (1) {
        ...
        //---------------------------------------------------
        // Detectar flanco ascendente botón BAR
        //---------------------------------------------------
        if (level_bar == 1 && last_level_bar == 0) {

            if (bar_press_count == 0) {
                // Primer pulso → iniciar ventana
        ...
        //---------------------------------------------------
        // Si pasó suficiente tiempo, decidir qué crear
        //---------------------------------------------------
        if (bar_press_count > 0 &&
            (now - first_press_time) >= pdMS_TO_TICKS(MULTI_PRESS_WINDOW_MS))
        {
            crear_barco_por_pulsos(bar_press_count);

...
static int _peek_prioridad(entrada_t *q, int head, int size)
{
...
static barco_t *_deq_max(entrada_t *q, int *head, int *size)
{
    ...
    int max_idx = *head;
    int max_vel = q[*head].prioridad;

    for (int i = 1; i < *size; i++) {
        int idx = (*head + i) % MAX_Q;
        if (q[idx].prioridad > max_vel) {
            max_vel = q[idx].prioridad;
            max_idx = idx;
        }
    }
...
static barco_t *prio_next(void)
{
    ...
    int vel_izq = _peek_prioridad(q_left,  ql_head, ql_size);
    int vel_der = _peek_prioridad(q_right, qr_head, qr_size);
